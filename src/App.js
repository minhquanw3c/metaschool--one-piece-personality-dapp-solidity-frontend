...
function App() {
	...
	//Initialize audio
	const [playBgSound, { stop: stopBgSound }] = useSound(bgAudio, {
		loop: true,
		volume: 0.2,
	});
	const [playLuffySound, { stop: stopLuffySound }] = useSound(luffyAudio, {
		loop: false,
	});
	const [playSanjiSound, { stop: stopSanjiSound }] = useSound(sanjiAudio, {
		loop: false,
	});
	const [playZoroSound, { stop: stopZoroSound }] = useSound(zoroAudio, {
		loop: false,
	});
	const [playUsoppSound, { stop: stopUsoppSound }] = useSound(usoppAudio, {
		loop: false,
	});
	const [playBrookSound, { stop: stopBrookSound }] = useSound(brookAudio, {
		loop: false,
	});
	...
	const checkMinted = async () => {
		setLoading(true);
		const bool = await nftcontract.methods
			.userHasAdoptedCat(account)
			.call();
		if (bool) {
			setMinted(true);
			fetchURI();
	...
	const checkMintedEvent = async (minter) => {
		...
		const checkEvent = async () => {
			try {
				...
				console.error(
					"Error occurred while querying the subgraph:",
					error
				);
				setLoading(false);
	...
	const fetchURI = async () => {
		...
		const tokenID = await nftcontract.methods
			.s_userToAdoptedCat(account)
			.call();
		const metadataIpfsLink = await nftcontract.methods
			.tokenURI(tokenID)
			.call();
							...
							<>
								{Minted ? (
									...
									<PersonalityForm
										...
										showForm={setShowPersonalityForm}
									/>
								) : (
